...
namespace threedi\steamsuite;

/**
 * Steam suite Extension base
 */
class ext extends \phpbb\extension\base
{
	...
	public function is_enableable()
	{
		...
		$user->add_lang_ext('threedi/steamsuite', 'ext_require');
		$lang = $user->lang;

		if ( !(phpbb_version_compare(PHPBB_VERSION, '3.2.1', '>=') || (phpbb_version_compare(PHPBB_VERSION, '3.1.9', '>=') && phpbb_version_compare(PHPBB_VERSION, '3.2.0@dev', '<'))) )
		{
			$lang['EXTENSION_NOT_ENABLEABLE'] .= '<br>' . $user->lang('ERROR_MSG_319_320_MISTMATCH');
			$is_enableable = false;
		}
		if (!function_exists('simplexml_load_string'))
		{
			$lang['EXTENSION_NOT_ENABLEABLE'] .= '<br>' . $user->lang('ERROR_MSG_SIMPLEXML');
			$is_enableable = false;
		}
		if (!function_exists('iconv'))
		{
			$lang['EXTENSION_NOT_ENABLEABLE'] .= '<br>' . $user->lang('ERROR_MSG_ICONV');
			$is_enableable = false;
		}
		if (!function_exists('curl_version'))
		{
			$lang['EXTENSION_NOT_ENABLEABLE'] .= '<br>' . $user->lang('ERROR_MSG_CURL');
			$is_enableable = false;
		}
		$user->lang = $lang;
	 ...
	 *
	 * @param bool $is_rhea Indicates if the version is above 3.2 or not
	 * @return void
	 */
